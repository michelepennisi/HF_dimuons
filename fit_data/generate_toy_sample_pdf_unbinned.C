...
void generate_toy_sample_pdf_unbinned(Int_t Low_Mass = 4, Int_t High_Mass = 30, Int_t Low_Pt = 0, Int_t High_Pt = 30, Double_t N_Events = 100000)
{
  ...
  // TCanvas *test = printMC_ratio(m_histo[i], m_Func[i], color[i]);

  Double_t pt_total_entries = pt_tree_MC[0]->GetEntries() + pt_tree_MC[1]->GetEntries() + pt_tree_MC[2]->GetEntries();
  Double_t m_total_entries = m_tree_MC[0]->GetEntries() + m_tree_MC[1]->GetEntries() + m_tree_MC[2]->GetEntries();

  Double_t pt_n_charm_frac = pt_tree_MC[0]->GetEntries() / pt_total_entries;
  Double_t pt_n_beauty_frac = pt_tree_MC[1]->GetEntries() / pt_total_entries;
  Double_t pt_n_mixed_frac = pt_tree_MC[2]->GetEntries() / pt_total_entries;

  Double_t m_n_charm_frac = m_tree_MC[0]->GetEntries() / m_total_entries;
  Double_t m_n_beauty_frac = m_tree_MC[1]->GetEntries() / m_total_entries;
  Double_t m_n_mixed_frac = m_tree_MC[2]->GetEntries() / m_total_entries;

  TFile *fIn_data = new TFile("~/dimuon_HF_pp/data/LHC18p/Hist_AOD/3_11_2022/TreeResults_merged.root", "READ");
  TTree *tree_fromdata = (TTree *)fIn_data->Get(Form("rec_data_tree%s", mass_range.Data()));
  /*/
  Double_t pt_n_charm_input = tree_fromdata->GetEntries() * pt_n_charm_frac;
  Double_t pt_n_beauty_input = tree_fromdata->GetEntries() * pt_n_beauty_frac;
  Double_t pt_n_mixed_input = tree_fromdata->GetEntries() * pt_n_mixed_frac;

  Double_t m_n_charm_input = tree_fromdata->GetEntries() * pt_n_charm_frac;
  Double_t m_n_beauty_input = tree_fromdata->GetEntries() * pt_n_beauty_frac;
  Double_t m_n_mixed_input = tree_fromdata->GetEntries() * pt_n_mixed_frac;

  */

  Double_t pt_n_charm_input = N_Events * pt_n_charm_frac;
  Double_t pt_n_beauty_input = N_Events * pt_n_beauty_frac;
  Double_t pt_n_mixed_input = N_Events * pt_n_mixed_frac;

  Double_t m_n_charm_input = N_Events * pt_n_charm_frac;
  Double_t m_n_beauty_input = N_Events * pt_n_beauty_frac;
  Double_t m_n_mixed_input = N_Events * pt_n_mixed_frac;

  RooDataSet *dataDimuMassFromBeauty = pdfDimuM[1]->generate(*m, m_n_beauty_input);
  RooDataSet *dataDimuPtFromBeauty = pdfDimuPt[1]->generate(*pt, pt_n_beauty_input);

  RooDataSet *dataDimuMassFromMixed = pdfDimuM[2]->generate(*m, m_n_mixed_input);
  RooDataSet *dataDimuPtFromMixed = pdfDimuPt[2]->generate(*pt, pt_n_mixed_input);

  RooDataSet *Mass_toyMC = pdfDimuM[0]->generate(*m, m_n_charm_input);
  RooDataSet *Pt_toyMC = pdfDimuPt[0]->generate(*pt, pt_n_charm_input);

  // dataDimuPtFromBeauty->Print();
  ...
  // Hist_pdfDimuPtFromCharm->SetMinimum(0.1);
  // Hist_pdfDimuPtFromCharm->Draw("PE");
  w->import(*Mass_toyMC);
  w->import(*Pt_toyMC);

  // w->writeToFile(Form("rooWorkspace_unbinned%s_Nev_%0.0f.root", mass_range.Data(),N_Events));
  w->writeToFile(Form("rooWorkspace_unbinned%s.root", mass_range.Data()));
  ...
  // pdfDimuM[i] = w->pdf(Form("pdfDimuMassFrom%s", name_DiMuSelection[i].Data()));
  // pdfDimuM[i]->fitTo(*M_Dimu[i]);
  // // RooFitResult *r = pdfDimuM[i]->fitTo(*M_Dimu[i], Range("pippo"), NormRange("region2"), Save());
  // pdfDimuM[i]->plotOn(frameDimuMass[i], Name(Form("pdfDimuMFrom%s", name_DiMuSelection[i].Data())), LineStyle(kSolid), LineColor(color[i]));
  Int_t i = 2;
  new TCanvas();
  // frameDimuPt[i]->Print();
  ...
  frameDimuMass[i]->Draw();

  // prova->Print();

  // RooCurve *central = frameDimuPt[i]->getCurve("pdfDimuPtFrommixed");
  // RooCurve *curve = frameDimuPt[i]->getCurve("pdfDimuPtFrommixed");
  ...
  // float upperValue = upBound->Eval(7.0);
  // float underValue = loBound->Eval(7.0);

  // printf("%0.10f || %0.10f \n",upperValue,underValue);
  // // upBound->Draw();
  // central->Draw();
  // // loBound->Draw("SAME");

  // RooCurve *central2 = frameDimuPt[i]->getCurve("pdfDimuPtFrommixed");
  // RooCurve *curve2 = frameDimuPt[i]->getCurve("pdfDimuPtFrommixed");
  // TGraph *upBound2 = new TGraph(central2->GetN());
  // TGraph *loBound2 = new TGraph(central2->GetN());

  // for (int j = 0; j < curve2->GetN(); ++j)
  // {
  //   if (j < central2->GetN())
  //     upBound2->SetPoint(j, curve2->GetX()[j], curve2->GetY()[j]);
  //   else
  //     loBound2->SetPoint(j, curve2->GetX()[j], curve2->GetY()[j]);
  // }

  // float upperValue2 = upBound2->Eval(7.0);
  // float underValue2 = loBound2->Eval(7.0);

  // printf("%0.10f || %0.10f \n",upperValue2,underValue2);
  // central2->Draw("SAME");

  // pt_histo[0]->GetListOfFunctions();
  // TRatioPlot *rp = new TRatioPlot(pt_histo[0]);
  // rp->Draw();
  // new TCanvas();
  // TH1 *histo = Pt_Dimu[i]->createHistogram("histo", *pt, Binning(30, 0, 30));
  // histo->Scale(1. / histo->Integral(), "width");
  // pt_histo[i]->Scale(1. / pt_histo[i]->Integral(), "width");
  // TH1 *histo1 = M_Dimu[i]->createHistogram("histo1", *m, Binning(12, 4, 30));
  // histo1->Scale(1. / histo1->Integral(), "width");

  // // auto pdfDimuPtFromCharm = w->pdf("pdfDimuPtFromCharm");
  // // pdfDimuPtFromCharm->fitTo(Pt_Charm);

  // // histo->Divide(pt_Func_charm);
  // histo->SetMarkerStyle(20);
  // histo->SetMarkerColor(kRed);
  // histo->Draw();
  // pt_histo[i]->Draw("PESAME");
  // pt_Func_charm->Draw("LSAME");
  // new TCanvas();
  // histo1->Divide(pt_Func_charm);
  // histo1->SetMarkerStyle(20);
  // histo1->SetMarkerColor(kRed);
  // histo1->Draw("SAME");

  // pdfDimuPtFromCharm->plotOn(frameDimuPt_total, Name("pdfDimuPtFromCharm"), LineStyle(kSolid), LineColor(kMagenta + 2));

  // TH1F *hsqrt1 = (TH1F *)gDirectory->Get("histo");

  // TCanvas *c1 = new TCanvas("c1", "c1");
  // c1->cd();
  // hsqrt1->Divide(pt_Func_charm);
  // hsqrt1->Draw("P");
}
//---------------------------------------------------------------------------------------//
