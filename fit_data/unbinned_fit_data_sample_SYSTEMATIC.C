...
// Fit data to value the systematic uncertainty of signal extraction
...
void yield_estimation(Int_t choice = 0, TString signal_param = "scaled_Low2Up", Int_t Binning_m = 26, Int_t Binning_pt = 30);

void unbinned_fit_data_sample_SYSTEMATIC()
{
  yield_estimation(0, "unbinned", 26, 30);
  yield_estimation(0, "original", 26, 30);
  yield_estimation(0, "scaled_Up2Low", 26, 30);
  yield_estimation(0, "scaled_Low2Up", 26, 30);
}
...
void yield_estimation(Int_t choice = 0, TString signal_param = "scaled_Low2Up", Int_t Binning_m = 26, Int_t Binning_pt = 30)
{
  ...
  TFile *fIn = new TFile("workspaces_systematic_pdf.root");

  RooWorkspace *w = (RooWorkspace *)fIn->Get(Form("w_%s", signal_param.Data()));
  w->Print();
  ...
  RooWorkspace *w_forMixed = (RooWorkspace *)fIn->Get("w_unbinned");
  w_forMixed->Print();
  ...
  RooAbsPdf *pdfDimuMassFromMixed = w_forMixed->pdf("pdfDimuMassFrommixed");
  RooAbsPdf *pdfDimuPtFromMixed = w_forMixed->pdf("pdfDimuPtFrommixed");
  ...
  TString info;
  info.Form("fit_data_systematic_%s_optfit%d", signal_param.Data(), choice);
