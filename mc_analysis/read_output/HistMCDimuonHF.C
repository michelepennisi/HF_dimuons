...
void HistMCDimuonHF(
    ...
    TString dir_fileout = "/home/michele_pennisi/dimuon_HF_pp/data/LHC18p/Hist_MC/14_10_22",
    TString prefix_filename = "MCDimuHFTree")
{
  ...
  TString fileout;
  // fileout.Form("%s/%s/Hist_%s", dir_fileout.Data(), RunMode, filename.Data());
  fileout.Form("%sHist%s", RunMode, filename.Data()); // test
  ...
  for (Int_t i = 0; i < input_tree->GetEntries(); i++)
  {
    ...
    for (Int_t i = 0; i < NDimu_rec; i++)
    {
      ...
      */
      ...
      for (Int_t DiMu_a = 0; DiMu_a < n_Mass_Cut; DiMu_a++)
      {
        for (Int_t DiMu_b = counter_rec; DiMu_b < n_DiMuCut; DiMu_b++)
        {
          for (Int_t DiMu_c = 0; DiMu_c < n_DiMu_Charge; DiMu_c++)
          {
            for (Int_t DiMu_d = 0; DiMu_d < n_DiMuSelection; DiMu_d++)
