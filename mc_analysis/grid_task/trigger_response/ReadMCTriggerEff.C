...
void ReadMCTriggerEff(
    const char *RunMode = "full",
    Int_t RunNumber = 294883,
    TString MC_type = "HF",
    ...
    Bool_t gridMerge = kTRUE)
{
  ...
  AliAnalysisManager *mgr = new AliAnalysisManager(Form("Trigger efficiency study %s Simulation", MC_type.Data()));
  ...
  // Add task
  //------------------------------------------------------------------------------------
  gROOT->LoadMacro("AliAnalysisTaskTriggerEff.cxx++g");
  AliAnalysisTaskTriggerEff *TriggerEffTask = reinterpret_cast<AliAnalysisTaskTriggerEff *>(gInterpreter->ProcessLine(Form(".x %s(\"%s\",%d)", "./AddTaskDimuonHF.C", MC_type.Data(),RunNumber))); // I set by hand usePhysicsSelection=kTRUE
  mgr->AddTask(TriggerEffTask);
  if (usePhysicsSelection)
  {
    TriggerEffTask->SelectCollisionCandidates(AliVEvent::kAny);
  }
