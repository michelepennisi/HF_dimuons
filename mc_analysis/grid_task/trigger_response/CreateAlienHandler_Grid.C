...
AliAnalysisGrid *CreateAlienHandler_Grid(const char *runMode, TString GridDir, TString MC_type, TString DataPattern, Int_t RunNumber, TString AliPhysicsVersion, Bool_t gridMerge)
{
  ...
  TString outdirname;
  outdirname.Form("%s_MC_TriggerResponse/%d", Period.Data(), RunNumber);
  ...
  plugin->SetGridOutputDir("");          // In this case will be $HOME/work/output
  ...
  // added by me
  plugin->SetAnalysisSource("AliAnalysisTaskTriggerEff.cxx");
  plugin->SetAdditionalLibs("AliAnalysisTaskTriggerEff.cxx AliAnalysisTaskTriggerEff.h AddTaskDimuonHF.C");
