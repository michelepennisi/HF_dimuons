    ...
    Bool_t gridMerge = kTRUE)
{
  ...
  // Add task
  //------------------------------------------------------------------------------------
  gROOT->LoadMacro("../AliAnalysisTaskDimuonPythia.cxx++g");
  AliAnalysisTaskDimuonPythia *MCTask = reinterpret_cast<AliAnalysisTaskDimuonPythia *>(gInterpreter->ProcessLine(Form(".x %s(\"%s\",%d)", "../AddTaskDimuonPythia.C", MC_type.Data(),RunNumber))); // I set by hand usePhysicsSelection=kTRUE
  mgr->AddTask(MCTask);
