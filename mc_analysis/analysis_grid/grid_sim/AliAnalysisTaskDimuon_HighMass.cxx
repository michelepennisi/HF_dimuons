...
void AliAnalysisTaskDimuon_HighMass::UserCreateOutputObjects()
{
    ...
    if (fSaving_opt.Contains("skimmed"))
    {
        ...
        if (fAOD_origin.Contains("Powheg"))
...
void AliAnalysisTaskDimuon_HighMass::UserExec(Option_t *)
{
    ...
    for (Int_t i_nMCpart = 0; i_nMCpart < n_MC_part; i_nMCpart++)
    {
        ...
        if (Muon)
        {
            if (fSaving_opt.Contains("skimmed_fwd"))
                if (MC_part0->Eta() < -4.2 || MC_part0->Eta() > -2.3)
                    continue;
        ...
        else if (Gamma_star)
        {
            ...
            if (index_Mum_MC_part0 == -1 && fSaving_opt.Contains("DY"))
            {
                printf("\n");
                printf("index %d\n", i_nMCpart);
                MC_part0->Print();
        ...
        else if (HF_quark && fSaving_opt.Contains("withHF_Q"))
        {
            ...
            // {
            //     dauther1 = (AliAODMCParticle *)mcarray->At(MC_part0->GetDaughterFirst());
            //     PDG_daugther1 = dauther1->GetPdgCode();
            // }
            // AliAODMCParticle *dauther2;
            // Int_t PDG_daugther2 = 999;
            // if (MC_part0->GetDaughterLast() > -1)
            // {
            //     dauther2 = (AliAODMCParticle *)mcarray->At(MC_part0->GetDaughterLast());
            //     PDG_daugther2 = dauther2->GetPdgCode();
            // }
            // printf("FIND HF QUARK\n");
            // printf("i_nMCpart %d) PDG: %d | MCParticle Pt %0.1f | Px %0.1f | Py %0.1f | Pz %0.1f | Y %0.1f || Mother %d , PDG Mother %d \n", i_nMCpart, PDG_MC_part0, MC_part0->Pt(), MC_part0->Px(), MC_part0->Py(), MC_part0->Pz(), MC_part0->Y(), index_Mum_MC_part0, PDG_Mum_MC_part0);
            // printf("Daughter1 HF %d || Daughter2 HF %d\n", PDG_daugther1, PDG_daugther2);
            // if (!(TMath::Abs(PDG_daugther1) > 100 && TMath::Abs(PDG_daugther2) > 100))
            //     continue;
        ...
        else if ((Charm_Hadron || Beauty_Hadron) && fSaving_opt.Contains("withHF_hadr"))
        {
            // MC_part0->Print();
            // printf("i_nMCpart %d) PDG: %d | MCParticle Pt %0.1f | Px %0.1f | Py %0.1f | Pz %0.1f | Y %0.1f || Mother %d , PDG Mother %d \n", i_nMCpart, PDG_MC_part0, MC_part0->Pt(), MC_part0->Px(), MC_part0->Py(), MC_part0->Pz(), MC_part0->Y(), index_Mum_MC_part0, PDG_Mum_MC_part0);
        ...
        for (Int_t j_nMCpart1 = i_nMCpart + 1; j_nMCpart1 < n_MC_part; j_nMCpart1++)
        {
            ...
            if (fSaving_opt.Contains("skimmed_fwd"))
                if (MC_part1->Eta() < -4.2 || MC_part1->Eta() > -2.3)
                    continue;
    ...
    for (Int_t i_Track0 = 0; i_Track0 < numtracks; i_Track0++)
    {
        ...
        fInitial_Parton_rec[nmu_rec] = muon_properties[2];
...
Int_t *Muon_ancestor_features(AliAODMCParticle *mcp_mumum, TClonesArray *mcarray, TString AOD_origin, Bool_t Muon_Verbosity, Bool_t Hadron_Verbosity)
{
    ...
    if ((TMath::Abs(PDG_muon_mum) > 400 && TMath::Abs(PDG_muon_mum) < 500) || (TMath::Abs(PDG_muon_mum) > 4000 && TMath::Abs(PDG_muon_mum) < 5000))
    {
        ...
        while (testing)
        {
            ...
            if (Hadron_Verbosity)
                printf("Round %d | Index Charm Hadron Ancestor %d | PDG Charm Hadron Ancestor %d | \n", round, index_ancestor_mum, PDGcode_ancestor0);
            ...
            else if (TMath::Abs(PDGcode_ancestor0) == 5)
            {
                ...
                if (Hadron_Verbosity)
                    printf("Hadron from Beauty Quark \n");
                testing = kFALSE;
                break;
            }
            else if (TMath::Abs(PDGcode_ancestor0) > 500 && TMath::Abs(PDGcode_ancestor0) < 600)
            {
                ...
                if (Hadron_Verbosity)
                    printf("Hadron from Beauty Meson \n");
            ...
            else if (TMath::Abs(PDGcode_ancestor0) > 5000 && TMath::Abs(PDGcode_ancestor0) < 6000)
            {
                ...
                if (Hadron_Verbosity)
                    printf("Hadron from Beauty Barion \n");
    ...
    else
    {
        ...
        while (testing)
        {
            ...
            if (Hadron_Verbosity)
                printf("Round %d | Index Mum %d | PDG Hadron Ancestor %d | \n", round, index_ancestor_mum, PDGcode_ancestor0);
