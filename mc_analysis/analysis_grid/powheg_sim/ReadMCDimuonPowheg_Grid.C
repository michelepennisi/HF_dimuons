...
void ReadMCDimuonPowheg_Grid(
    const char *RunMode = "test",
    Int_t RunNumber = 294009,
    TString Version = "Version_test",
    TString MC_type = "powheg_charm",
    ...
    Bool_t gridMerge = kTRUE)
{
  ...
  AliAnalysisGrid *alienHandler = reinterpret_cast<AliAnalysisGrid *>(gInterpreter->ProcessLine(Form("CreateAlienHandler_Grid(\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",%d,\"%s\",%d)", RunMode, Version.Data(), GridDir.Data(), MC_type.Data(), DataPattern.Data(), RunNumber, AliPhysicsVersion.Data(), gridMerge)));
  if (!alienHandler)
    return;
  ...
  // Add task
  //------------------------------------------------------------------------------------
  gROOT->LoadMacro("AliAnalysisTaskDimuonPowheg.cxx++g");
  AliAnalysisTaskDimuonPowheg *MCTask = reinterpret_cast<AliAnalysisTaskDimuonPowheg *>(gInterpreter->ProcessLine(Form(".x %s(\"%s\",%d)", "../AddTaskDimuonPowheg.C", MC_type.Data(), RunNumber))); // I set by hand usePhysicsSelection=kTRUE
  mgr->AddTask(MCTask);
