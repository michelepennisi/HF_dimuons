...
AliAnalysisGrid *CreateAlienHandler_Grid(const char *runMode, TString Version, TString GridDir, TString MC_type, TString DataPattern, Int_t RunNumber, TString AliPhysicsVersion, Bool_t gridMerge)
{
  ...
  printf("OOOOOOOOOO: %s\n", Form("%s/%s/LHC18p", GridDir.Data(), MC_type.Data()));
  plugin->SetGridDataDir(Form("%s/%s/LHC18p", GridDir.Data(), MC_type.Data())); // Data
  ...
  // added by me
  plugin->SetAnalysisSource("AliAnalysisTaskDimuonPowheg.cxx");
  plugin->SetAdditionalLibs("AliAnalysisTaskDimuonPowheg.cxx AliAnalysisTaskDimuonPowheg.h AddTaskDimuonPowheg.C");
